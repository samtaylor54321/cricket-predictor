    ...
    for innings_idx, innings in enumerate(data.get("innings", [])):
        ...
        for over in innings.get("overs", []):
            for delivery in over.get("deliveries", []):
                ...
                if batter:
                    batting_runs[batter] = batting_runs.get(batter, 0) + runs.get(
                        "batter", 0
                    )
                ...
                if bowler:
                    ...
                    bowling_runs[bowler] = bowling_runs.get(bowler, 0) + runs.get(
                        "total", 0
                    )
...
def run(
    ...
    for i, filepath in enumerate(json_files):
        ...
        try:
            ...
            player_rows.extend(parse_player_performances(data, match["match_id"]))
    ...
    matches_df = (
        pd.DataFrame(match_rows).sort_values("match_date").reset_index(drop=True)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Parse Cricsheet county championship JSON files"
    )
