                        ...
                        "deliveries": [
                            {
                                ...
                                "non_striker": "JE Root",
                                "runs": {"batter": 0, "extras": 0, "total": 0},
                                "wickets": [{"kind": "lbw", "player_out": "AD Hales"}],
                            },
                            {
                                "batter": "JE Root",
                                "bowler": "DA Payne",
...
class TestParseOutcome:
    ...
    def test_winner_by_innings(self):
        winner, result = parse_outcome(
            {"winner": "Essex", "by": {"innings": 1, "runs": 34}}
        )
        assert winner == "Essex"
        assert result == "decisive"
...
class TestParsePlayerPerformances:
    ...
    def test_wicket_credited_to_bowler(self, sample_match_data):
        ...
        coad = next(
            r for r in rows if r["player_name"] == "BO Coad" and r["innings"] == 1
        )
    ...
    def test_wicket_credited_to_bowler_innings_2(self, sample_match_data):
        rows = parse_player_performances(sample_match_data, "test_123")
        payne = next(
            r for r in rows if r["player_name"] == "DA Payne" and r["innings"] == 2
        )
...
class TestRun:
    ...
    def test_decisive_draw_split(self, tmp_path, sample_match_data, draw_match_data):
        ...
        assert (
            matches_df.loc[matches_df["result_type"] == "decisive", "home_win"].iloc[0]
            == 1
        )
        assert (
            matches_df.loc[matches_df["result_type"] == "draw", "home_win"].isna().all()
        )
